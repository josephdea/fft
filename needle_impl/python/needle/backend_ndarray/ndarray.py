...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        assert prod(new_shape) == prod(self._shape)
        return NDArray.make(shape=new_shape,
                            strides=None,  # will call compact_strides
                            device=self._device,
                            handle=self._handle,
                            offset=self._offset)
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        return NDArray.make(shape=tuple(self._shape[x] for x in new_axes),
                            strides=tuple(self._strides[x] for x in new_axes),
                            device=self._device,
                            handle=self._handle,
                            offset=self._offset)
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        """
        ### BEGIN YOUR SOLUTION
        temp_strides = list(self._strides)
        for i, (n, o) in enumerate(zip(new_shape, self._shape)):
            if n != o:
                assert o == 1
                temp_strides[i] = 0
        return NDArray.make(shape=new_shape,
                            strides=tuple(temp_strides),
                            device=self._device,
                            handle=self._handle,
                            offset=self._offset)
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        while len(idxs) != self.ndim:
            idxs.append(self.process_slice(slice(None, None, None), len(idxs)))
        ...
        for stride, idx in zip(self._strides, idxs):
            new_offset += stride * idx.start
            new_strides.append(stride * idx.step)
            new_shape.append((idx.stop - idx.start - 1) // idx.step + 1)
        return NDArray.make(shape=new_shape,  # tuple(new_shape) is called in `make`
    ...
    def flip(self, axes):
        ...
        for axis in axes:
            strides[axis] *= -1
            if axis == len(strides) - 1:
                offset += self._shape[-1] - 1
            else:
                offset += (self._shape[axis] - 1) * prod(self._shape[axis + 1:])

        return NDArray.make(shape=self._shape,
    ...
    def pad(self, axes):
        ...
        padded = full(new_shape, 0, device=self.device)
        slices_for_original = tuple([slice(None, None, 1)] * len(self.shape))
        slices_for_padded = []
        for i, axis in enumerate(axes):
            l_pad, r_pad = axis
            start = l_pad
            end = new_shape[i] - r_pad
            slices_for_padded.append(slice(start, end, 1))
        
        padded[tuple(slices_for_padded)] = self[slices_for_original]
        return padded
        ### END YOUR SOLUTION

    def _invoke_naive_fourier_1d(self, fn):
        assert len(self.shape) == 1
        real_out = self.device.full(self.shape, 0, "float32")
        imag_out = self.device.full(self.shape, 0, "float32")
        fn(self.compact()._handle, real_out._handle, imag_out._handle, self.shape[0])
        return real_out, imag_out

    def naive_forward_fourier_1d(self):
        return self._invoke_naive_fourier_1d(self.device.naive_forward_fourier_1d)

    def naive_backward_fourier_1d(self):
        return self._invoke_naive_fourier_1d(self.device.naive_backward_fourier_1d)
...
def swapaxes(a, axis_1, axis_2):
    new_dims = list(range(len(a.shape)))
    new_dims[axis_1], new_dims[axis_2] = new_dims[axis_2], new_dims[axis_1]
    return a.permute(new_dims)

def squeeze(a, axis):
    new_shape = []
    if axis is None:
        axis = range(len(a.shape))
...
def forward_fourier_1d(a):
    return a.naive_forward_fourier_1d()

def backward_fourier_1d(a):
    return a.naive_backward_fourier_1d()
